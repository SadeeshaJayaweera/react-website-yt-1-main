...
const BoardingFilter = ({ onFilterChange }) => {
    ...
    const handleFilterChange = () => {
        ...
        const validatedRatingFilter = Math.max(1, Math.min(5, parseFloat(ratingFilter))) || ''; // Ensure rating is between 1 and 5
        const validatedDistanceFilter = Math.max(0, parseFloat(distanceFilter)) || ''; // Ensure distance is non-negative
